...
#include<time.h>	 // temp per i test
#include<sys/time.h> // temp per i test
//#include<fcntl.h>
struct timeval start_time, stop_time, res_time; // temp per i test
...
//!upload /Users/Damiano/filetest/100M.txt 12345678901234567890123456789012
...
void calcolofinale(struct job* job)
{
    ...
    timersub(&stop_time, &start_time, &res_time);
    
    printf("\nTempo: %ld.%06ld\n", (long int)res_time.tv_sec, (long int)res_time.tv_usec);
	EVP_CIPHER_CTX_cleanup(ctx);
	EVP_CIPHER_CTX_free(ctx);
